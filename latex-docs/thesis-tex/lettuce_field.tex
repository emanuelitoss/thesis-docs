...
\lettrine[lines=2, findent=3pt, nindent=0pt]{T}{}he previous chapter described the general workspace of lattice QCD with particular attention to the regularizations used in the present work.
...
\begin{enumerate}
    \item An automatic \oait\space of the three points correlators needed to extract matrix elements $\la \bar K^0 | \Theta_i^{[+]} | K^0 \ra$ without the need of multiple simulations (Wilson average) or other specific strategies.
...
\noindent
In order to achieve these properties, some reformulations of the operators and strategies are needed.
In the first part (and most) of the chapter I describe the four fermions, dimension six, mizing operators on the lattice.
The second part of the chapter is reserved to the description of the strategy to extract matrix elements $\la \bar K^0 | \Theta_i^{[+]} | K^0 \ra$ from two and three points correlators.
...
I introduce such correlators\footnote{To be precise, these are not the usual $n-$points correlators but their projections over null momenta.} in continuum QFT:
...
\end{equation*}
The source operators $\bar K^0$ and $K^0$ are defined in Chapter \ref{chap:kaons} as the pseudoscalar densities of $d$ and $s$ doublet of quarks, while the superscript QCD remembers that these are the usual correlation functions in continuum quantum chromodynamics.
...
The choice of multiple quarks of the same flavour comes from \cite{FR2} and it is formulated in order to achieve some properties of the three points correlators;
such properties are well described in \ref{sec:operators-properties}.
I refer to the model with flavour replicas with the symbol FR (::Flavour Replicas).
\newline
