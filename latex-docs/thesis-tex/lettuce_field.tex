...
The prpjection over zero momenta allows to simplify the expression.
...
\newline\newline
The Dirac matrices are the one of the so called ``chiral representation''\cite{Itzykson-Zuber} and Lorentz generators for spinors are the following:
\begin{equation*}
    \begin{gathered}
    ...
    \end{gathered}
\end{equation*}
In this case the Minkowski and eucledian Lagrangian densities are:
\begin{equation*}
    \begin{gathered}
...
\end{equation*}
and a short notation is $X^{12} = \bar \psi^1 \Gamma^\alpha \psi^2$ where $X = S, V, P, A, T, \tilde{T}$.
    ...
    \item [-] Properies of the left and right Weyl projectors:
        \begin{equation*}
            \begin{gathered}
                ...
                P_L \gamma_\mu = \gamma_5 P_R \quad P_R \gamma_\mu = \gamma_\mu P_L \quad \{ P_{L,R},\gamma_5 \} = 0
             \end{gathered}
        \end{equation*}
\end{itemize}
Then:
...
\end{equation*}
The term $\gamma_5 S_{(\pm)} (z,y) \gamma_5$ it is clearly $S_{(\mp)}(y,z)$ for definition.
\begin{equation*}
    \hspace*{12cm}\square
\end{equation*}
