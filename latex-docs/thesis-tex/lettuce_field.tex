...
And a dircet consequence of this {\it global} trivialization is that Gauge fields are globally defined over all the manifold $M$.
\newline
At this point, the proof is essetially complete.
I need to check that, given two Gauge fields $A_\mu^{(1)}$ and $A_\mu^{(2)}$, there exists always a smooth path in the fields space which connects them.
The proof is divided in three pieces:
\newline
\newline
...
I need to prove that the transformed field $A_\mu^s (x)$ satisfies the temporal gauge and the \obc\space again.
\begin{equation*}
...
\begin{itemize}
    ...
    \item \obc\space $\forall s \in [0,1]$: I must re-evaluate the strength tensor and use the property of $\Lambda_s (\vec x, x_4 = 0)$.
        There are a lot of calculations not reported here, but it is checked.
\end{itemize}
$(ii)$ \textit{Each $A_\mu$ in temporal Gauge can be contracted to the null Gauge field $A_\mu = 0$.}
\newline
This part of the proof is trivia: $A_\mu^s = s\cdot A_\mu$
\newline\newline
At this point it is proved that each $A_\mu$ satisfying \obc\space can be smoothly contracted to the null potential.
Then each couple of potentials $A_\mu^{(1)}$ and $A_\mu^{(2)}$ are connected.
\proved
