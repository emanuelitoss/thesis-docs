...
\begin{abstract}
    ...
    \newpage
    Le oscillazioni dei Kaoni sono un fenomeno ben conosciuto nel modello standard delle particelle \colr{ref}.
    Cosa vorrei inserire:
    \begin{itemize}
        \item intro storica alle oscillazioni dei kaoni: quando sono state scoperte/formulate, perché sono tanto utili (CP violation).
        \item QCD non perturbativa, confinamento e assenza di teorie esatte se non il path integral.
            Fattori non perturbativi, $B_K, f_K$.
        \item Studio del path integral tramite lattice, motivazioni. Regolarizzazioni. Problemi di improvement e autocorrelazioni.
            Regolarizzazione usata. Motivi.
        \item Utilizzzo della base di operatori. Oscillazioni BSM. Tutti i ghirigori che si fanno su questa base per arrivare alla nuova.
        \item Metodo dei noise spinors
        \item Check fatti, risultati, etc.
    \end{itemize}
\end{abstract}
...
\noindent
This section gives an overview about the analysis strategy used to recover the lattice amplitudes $\mathcal{A}_i (a)$.
Once recovered them for different lattice spacings $a$, a fit in the continuum limit $a\rightarrow 0$ will give the best estimate of $\mathcal{A}_i^\text{QCD}$.
The analysis is divided in some steps, that I briefly report below.

\subsubsection*{First step: determination of asymptotic distance $\delta_4$}
\noindent
...
To estimate it, I make use of two points correaltors.
...
In the last equality I fixed $z_4 = a$, option valid in the entire simulation.
I fix $z_4 = 1$ and run the simulation for all the $x_4 \in (z_4+a,T-a)$.
The option $x_4 = T$ will not produce physical results because of open boundary conditions.
The strategy consists in performing a fit of the hyperbolic sine in different regions of the space.
I start with the maximal region $[a, T-a]$ and decrease it step by step $[2a,T-2a], [3a,T-3a], \dots $ until the the fit is satisfactory - i.e. a good value of $\chi^2$.
The first satisfactory region gives the value of $\delta_4$: $[\delta_4, T-\delta_4]$.
At this point I have both determined $m_K$ as a parameter and the {\it minimal asymptotic distance} $\delta_4$.
...
I denote them with the $hat$ identifier to underline that thes data are the only I can use to recover the wanted amplitudes.

\subsubsection*{Second step: plateau region of three points correaltors}
\noindent
At this point I am ready to analyze three points correlators $C_i^\text{FR}(x_4,y_4,z_4)$.
I take a set of correaltors with $z_4 = a$, $x_4$ fixed with $x_4 > 2\delta_4 + a$, while $y_4$ varies in $[z_4,x_4]$.
...
A fit in this region is nothing but an average value:
\begin{equation*}
    \hat C_i^\text{FR}(x_4,z_4=a) = \frac{1}{N_\text{region}} \sum_\text{region} C_i^\text{FR}(x_4,y_4,a)
\end{equation*}
By repeating these steps and varying the value of $x_4$, I obtain a set of $\hat C_i^\text{FR}$.
...
By analyzing some observables of the gauge fields it is possible to recover the value of the lattice spacing $a$ associated to a set of configurations.
\newline
Repeating first, second and third step for different sets of Gauge fields configurations brings to a set of amplitudes for different values of the lattice spacing.
...
It consists in $(i)$ a fit of the values of the amplitudes versus their associated lattice spacing and $(ii)$
...
%\section{Proof of the measure invariance under twist}
%\noindent
%I want to prove that a twist transformation à la Osterwalder-Seiler or à la Twisted Mass has Jacobian equal to identity.
%In the following proof I choose an OS twist because it automatically implies the Twisted Mass case.
%\newline
%The twist transformation $\mathbf{J}(\alpha,r)$ has no dependence on the spacetime point $x$ or on the fields.
%Then:
%\begin{equation*}
%    \mathcal{D} \psi_i \mathcal{D} \bar\psi_i = \mathbf{J}^2(\alpha,r) \mathcal{D} \chi_i \mathcal{D} \bar\chi_i
%\end{equation*}
%So the Jacobian is $\mathbf{J}^2(\alpha,r) = \mathbf{J}(2\alpha,r)$, but it is a constant.
%Then, in the average of some observable $\mathcal{X}$ it simplifies with the term arising form $\mathcal{Z}$.
%Therefore I can state that the Jacobian {\it is} the identity, in the sense that it is a constant.
%Since this relation holds separately for each single flavour $i$, it holds also for two flavours in tmQCD.
...
\proved
%\section{OBC protect sea configurations against zero modes}\label{app:obc-zero-modes}
%\noindent
%I want to prove that \obc\space generate lower bound for the value of eigenvalues of Dirac-Wilson operator $\Lambda_i[U]$.
%The proof is developed in continuum field theory in a finite spacetime volume \cite{SF-luscher}.
%It is still valid in the case of lattice discretization.
