...
\lettrine[lines=2, findent=3pt, nindent=0pt]{T}{}he purpose of this chapter is to introduce the reader to the basics of lattice regularization and to describe the adopted one in this work.
\newline
...
As known, path integral is an \textit{infinite} sum of fields configurations defined over an \textit{infinite} volume and time extension.
Despite this, vacuum expectation values of physical observables are \textit{finite} quantities.
...
It consists into a discretization of spacetime - called lattice - and a consequent finite volume.
The two most relevant sources of errors are the discretization of the space and the finite volume effects.
To deal with these errors, a large number of improvements has been developed in past years and some of them are applied in this work.
\newline
In the first section I will expose generalites about lattice regularization.
...
In the fourth section a particular set of bosonic ghost fields are described.
At the end, the fifth section describe the lattice regularization used in the present simulation.

\section{Lattice regularization of QFT}\label{sec:lattice-discretization}
\noindent
A finite and discretized functional integral can be formulated by applying the following changes to the continous functional integral \cite{montvay-munster}\cite{gattringer-lang}:
\begin{itemize}
    \item [$\triangleright$] The Minkowski space $\mathbb{M}^4$ is replaced by an hypercubic lattice of $N_\text{time} \times N_\text{space}^3$ points.
        ...
        Thus the lattice space is defined by:
        \begin{equation*}
...
\end{itemize}
The set of the previous changes takes the name of \textit{lattice regularization} of a continuum theory.
...
The path integral provides theoretical tools to treat a non perturbative theory, then all the quantites in lattice simulations are evaluated through expectation values of some choosen observables.
\newline
In the case of QCD described in the previous chapter, the integration can be splitted in two pieces: the {\it fermionic integration} $\la \hspace*{0.5mm} \cdot \hspace*{0.5mm} \ra_F$ with measure $\mathcal{D}\psi\mathcal{D}\bar\psi$ and the {\it Gauge integration} $\la \hspace*{0.5mm} \cdot \hspace*{0.5mm} \ra_G$ with measure $\mathcal{D}U$.
...
Since physics resides in the continuum limit, there is freedom in the choice of the regularization.
All the valid regularizations share the same physical continuum limit, but they differ one from each other by $O(a^n)$ terms, with $n\ge 1$.
...
\lettrine[lines=2, findent=3pt, nindent=0pt]{T}{}he previous chapter described the general workspace of lattice QCD with particular attention to the regularization used in the present work.
This chapter describes the formulation on the lattice of the mixing operators $\{\Theta_i\}$ and focuses on some properties of such lattice operators.
...
\begin{enumerate}
    ...
    \item A specific blocks-like renormalization matrix $Z_{ij}$ that simplify the renormalization procedure.
    \item An automatic \oait\space of the three points correlators needed to extract matrix elements $\la \bar K^0 | \Theta_i^{[+]} | K^0 \ra$ without the need of multiple simulations (Wilson average).
\end{enumerate}

\noindent
In order to achieve these properties, some reformulations and strategies are needed.
The second part of the chapter is reserved to the description of the strategy to extract matrix elements $\la \bar K^0 | \Theta_i^{[+]} | K^0 \ra$ from three point correlators.
...
But first I need to present some changes to the old SUSY basis.
\newline
I want to rewrite operators $\Theta_3$ and $\Theta_5$ with the contractions of spin indices and colour indices within the same couple of quarks.
This can be done by applying the Fierz relations\cite{Itzykson-Zuber}, described in Appendix \ref{app:fierz} along with calculation of $\Theta_{3,5}$.
...
\begin{equation}\label{eq:Theta_i-final}
  ...
  \qquad\qquad
    ...
    \end{split}
\end{equation}
Now I introduce the new basis $\{Q_i^{[\pm]}\}$ \cite{DoniniMartinelliOperators}:
...
once regularized in the way explained in this chapter, they have renormalization mixing properties that simplify the evaluation of renormalized operators and renormalization constants.
\colr{eventualmente aggiungere anche il fatto che non c'è mixing PE PO chirale}
This will be better explained in \ref{sec:renormalization-properties}.
\newline
I will show how the matrix elements $\la \bar K^0 | Q_i^{[\pm]} | K^0 \ra$ can be asymptotically extracted from three points correlators.
I introduce such correlators here:
\begin{equation}\label{eq:3pts-correlators}
    C_i^\text{QCD}(x_4,y_4,z_4) = a^6\sum_{\vec x, \vec y, \vec z} \left\la \bar K^0(x) Q_i(y) \bar K^0(z) \right\ra
\end{equation}
The source operator $\bar K^0$ is defined in Chapter \ref{chap:kaons}, while the superscript QCD remembers that these are the usual correlation functions in continuum theory.
The amplitudes are recovered by taking its asymptotic behaviour for $0 \ll z_4 \ll y_4 \ll x_4 \ll T$,then the condition $x_4 > y_4 > z_4$ is supposed to be always satisfied.
...
The choice of multiple quarks of the same flavour comes from \cite{FR2} and it is formulated in order to ``unlock'' some properties of the three points correlators described in \ref{sec:operators-properties}.
I refer to the model with flavour doublers with the symbol FR (::Flavour Replicas).
\newline
...
In particular, I want to build three points correlators in FR model such that they give the same Wick contractions of functions \ref{eq:3pts-correlators}.
The proof is given in the below section.

\subsection{Proof of equivalence of Wick contractions}
\noindent
Let's consider a general three points correlator in standard model QCD:
\begin{equation*}
...
The correlaion functions \ref{eq:3pts-correlators} are nothing but the sum over spatial components of the above quantity.
The Wick contractions are the following\footnote{I use the indices $c,d$ to refer to connected or disconnected diagrams}:
\begin{equation*}
    \begin{split}
    ...
    \end{split}
\end{equation*}
They generates these single or double traces functions:
...
\end{equation*}
I want to replicate this contributions by using the new flavours $d,d',s,s'$ built on the lattice.
The strength of this method consists in the fact that primed and unprimed flavours cannot contract with each other, then I can build an appropriate operator for each contraction.
I define a source in $x$ with primed flavours $\bar d' (x) \Gamma_\xi s'(x)$ and an other in $z$ with unprimed flavours $\bar d (z) \Gamma_\omega s(z)$.
...
\noindent
Thanks to the previous proof, the parity even operators $Q_{i}^{[+]}$ (equation \ref{eq:Q_i-continuum}) can be built on the lattice in the FR framework.
...
\end{equation*}
The operators $Q_{i}^{[+]}$s are replaced by the new operators $O_{i,[+]}^\text{phys}$ in the fermion physical basis:
\begin{equation*}
    \begin{split}
...
Now the work in Maximally twisted OS valence QCD is well defined and, by construction, the correlators:
...
\end{equation}
gives the same processes of the functions in equation \ref{eq:3pts-correlators}.
...
In a shorthand notation I refer to the set of Wilson parameters of the theory through $R = \{r_1,\dots,r_N\}$.
\newline
...
The proof that the Wilson average gives an \oait\space is in \cite{FR1}. 
\newline
The WA method implies that multiple simulations must be done.
However there are particular cases in which this is not necessary because $\left\la \mathcal{X} \right\ra\big|_{\pm R}^a$ are automatically improved and the WA is not necessary.
...
\subsection{Absence of chiral mixing and renormalization constants $Z_{ij}$}\label{sec:renormalization-properties}
\noindent
\colr{questa ultima è ancora da fare bene}
...
\begin{equation*}
    \begin{split}
        ...
        & \text{where $i$ runs over all the particles in the given state.}
    \end{split}
\end{equation*}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
I insert a complete set of states between each pair of operators and I use the property
...
To evaluate matrix elements I will use $M_A = M_B = \bar K^0$ and $\Xi = O_{i[+]}$ with some properties\footnote{The used ones are: $m_A = m_B = m_K$ and $(K^0(x))^* = \bar K^0(x)$ from Appendix \ref{app:notations}.}.
The asymptotic behaviours of the given correlators are:
\begin{equation*}
    \begin{split}
        & C_i(x_4,y_4,z_4) \approx a^6\sum_{\vec x} \la \bar K^0 | O_{i[+]} | K^0 \ra  \la \Omega | \bar K^0 | \bar K^0 \ra  \la K^0 | \bar K^0 | \Omega \ra  \hspace*{.5mm} e^{-m_K(x_4-z_4)}  \\
        & G_{\bar K^0 K^0}(x_4,y_4) \approx a^3\sum_{\vec x} \Big| \la \Omega | \bar K^0 | \bar K^0 \ra \Big|^2 \hspace*{.5mm} e^{-m_K(x_4-y_4)} \\
        & G_{K^0\bar K^0}(x_4,y_4) \approx a^3\sum_{\vec x} \Big| \la \Omega |  K^0 |  K^0 \ra \Big|^2 \hspace*{.5mm} e^{-m_K(x_4-y_4)} \\
    \end{split}
\end{equation*}
...
I leave the proof of that at the end of the chapter.
...
\begin{equation}\label{eq:matrix-elements-extraction}
    \begin{split}
        & \text{On the lattice:} \quad \mathcal{A}_i (a) = \la \bar K^0 | O_{i[+]} | K^0 \ra \approx \frac{C_i (x_4,y_4,z_4)}{G_{\bar K^0 K^0}(x_4,z_4)} \\
        & \text{Continuum limit:} \quad \mathcal{A}_i (a) \xrightarrow{a \rightarrow 0} \mathcal{A}_i^\text{continuum} + O(a^2)
    \end{split}
\end{equation}

\subsection*{Proof that $\la \Omega | \bar K^0 (0) | \bar K^0 \ra$ is real:}
\noindent
From Goldstone theorem \cite{Goldstone-Theorem} I know that there exist some pseudoscalar single particle states $| \phi^a (\vec p) \ra$ and some local operators $\phi^a(x)$ that interpolate such states in the simple way $\la\Omega | \phi^a (x) | \phi^b (\vec p) \ra = \delta^{ab}e^{ipx} $.
In this case I will use $|\phi^a(\vec p)\ra = | K^0 (\vec 0)\ra \equiv | K^0 \ra$.
The operators $\phi^a$ are related, in QCD, to the vector axial currents by:
\begin{equation*}
    j_\mu^{\text{A},a} (x) = F_\pi^a \partial_\mu \phi^a (x)
    \quad \Rightarrow \quad
    \partial_\mu j_\mu^{\text{A},a} (x) = F_\pi^a m_{\pi^a}^2 \phi^a (x)
\end{equation*}
but the vector axial current it is also defined by $j_\mu^a(x) = \bar \psi^i (x) T^a_{ij}\gamma_\mu \gamma_5 \psi^j (x)$ where $T^a$ are SU(3) generators and $i,j$ are flavour indices.
...
\end{equation*}
Through a comparison of the four divergencies I obtain the equality:
\begin{equation*}
...
Now I remember that I use, as sources, the pseudoscalar densities $P^{ij}(x)$ and not the unknown operators $\phi^a (x)$, then my matrix elements are:
\begin{equation*}
    \la \Omega | \bar K^0 (0) |  \bar K^0 \ra = \la \Omega | K^0 (0) | K^0 \ra = \frac{F_K m_K^2}{m_s + m_d}
\end{equation*}
Since these are real, they are also equal to their conjugates $\la K^0  | \bar K^0 (0) |  \Omega\ra$ and $\la\bar K^0 | K^0 (0) | \Omega\ra$.
...
$$N = N_T \x N_L^3 \x N_\text{colour} \x 4 \x 2 $$
...
\noindent
The metric used in this work is the Eucledian metric $\delta_{\mu\nu}$.
In this case there is no distinction between upper and lower Dirac indices. 
Usually I prefer to use lower indices.
\newline\newline
The Dirac matrices used are the matrices of so called ``chiral representation'' \cite{Itzykson-Zuber} and Lorentz generators for spinors are the following:
\begin{equation*}
    \begin{gathered}
...
\begin{equation*}
    ...
    \end{gathered}
\end{equation*}
it follows the Dirac equation with its DIrac conjugate (valid both in Minkowski and Eucledian metrics):
\begin{equation*}
    \begin{gathered}
    ...
    \end{gathered}
\end{equation*}
Dirac bilinears are usually expressed in the following form: $\bar \psi^1 \Gamma^\alpha \psi^2$ with $\psi^1$ and $\psi^2$ two distinct flavours.
Dirac operators belong to this set:
\begin{equation*}
    \Gamma^{\alpha} \in \{\mathbb{I}_4,\gamma_\mu, \gamma_5, \gamma_\mu\gamma_5, \sigma_{\mu\nu}, \tilde\sigma_{\mu\nu} \}
\end{equation*}
and a short notation is $X^{12} = \bar \psi^1 \Gamma^\alpha \psi^2$ where $X = S, V, P, A, T, \tilde{T}$ indicates which matrix is choosen.
...
\noindent
In the context of twisted mass lattice QCD described in \ref{sec:tmLQCD}, it is possible to define a physical fermion basis by:
\begin{equation*}
    \chi = \mathbf{T}(\alpha)\hspace*{0.5mm} \psi \hspace*{1cm} \overline{\chi} = \bar\psi\hspace*{0.5mm}\mathbf{T}(\alpha)
\end{equation*}
where $\mathbf{T}(\alpha)$ is the twist rotation \ref{eq:twist}.
In this case the twisted mass action takes the following form:
...
\end{equation}
where the new bilinear operator is:
\begin{equation*}
...
\end{equation*}
The main ingredients to prove it follows:
\begin{enumerate}
    \item Remember that $(\gamma_5)^2=\mathbb{I}_4$ and $(\sigma^3)^2=\mathbb{I}_2$
...
\end{enumerate}
The new set of variables $\{\chi,\bar\chi\}$ is called {\it physical} basis because it does not involve the (unphysical) term $\mu_q$.
In other words, the mass is entirely rotated and contained in $M_q = \sqrt{m^2+\mu_q^2}$.
The twist rotation only affects the Wilson term, that results modified.
Despite this fact, it is known that the Wilson term disappear in the continuum limit, so the action \ref{eq:physical-action} tends to the physical continuum action.
Then it is the ``true'' discretization of the continuum theory and the twisted action defined in \ref{eq:tmQCD-action} is the modified action.
\newline
\noindent
The link between the two actions is the twist rotation of fermion fields and I will show that an expectation value can be evaluated in both theories.
Remember that the environment in which the theory is applied is the functional integral formalism, thus we must take care about integration measure.
The variable change:
\begin{equation*}
    \mathcal{D}\chi \hspace*{0.5mm} \mathcal{D}\overline{\chi} \mapsto \mathcal{D}\psi \hspace*{0.5mm} \mathcal{D}\overline{\psi}
\end{equation*}
it is not anomalous\footnote{i.e. it does not introduce measure anomalies} and the Jacobian is the identity.
Then I report the main result of this section: given an observable $\mathcal{X}$ field-dependent, its expectation value is the two formulations coincides:
\begin{equation*}
...
The strength of tmQCD theory lies in this variables replacement.
\newline
Now I report a list of some simple operators - currents and densities - and their transformation rules.
...
\begin{itemize}
    ...
    \end{cases}$
\end{itemize}
The very particular case of maximal twist ($\alpha = \pm \pi/2$) reads:
\begin{itemize}
...
\noindent
In the case of OS regularization, the physical and twisted basis are realted by the following:
\begin{equation*}\label{eq:OSbasischange}
    ...
    \end{cases}
\end{equation*}
By expanding in series the exponential, it can be proved that $\mathbf{J} (\alpha, r) = \mathbb{I}_4 \cos (\alpha r/2) + i \gamma_5 \sin (\alpha r/2)$.
The proof that the two actions reported in section \ref{sec:OS-regularization} are equivalent is trivial and based on the following identities:
\begin{equation*}
    \begin{gathered}
    ...
    \end{gathered}
\end{equation*}
Also in this case, the change of coordinates does not instroduce any anomalous term in the integration measure.
Moreover, the Jacobian of the transformation is the identity:
\begin{equation*}
    \mathcal{D} f \mathcal{D} \bar f = \mathcal{D} q \mathcal{D} \bar q
\end{equation*}
Also in this cases masses are rotated and the formula \ref{eq:new-masses} is still valid.
Then I can use \ref{eq:OSbasischange} to express some observables in the twisted basis at maximal twist $\alpha = \pi/2$.
...
\end{enumerate}
Let's start with some definitions, valid in both cases:
\begin{itemize}
...
\begin{equation*}
    \begin{aligned}
        & \text{PCAC: } \partial_\mu A_{\mu,12}^\text{tw} = (m_1+m_2)\left[\cos (\alpha) P_{12}^\text{tw} \pm i \sin (\alpha) S_{12}^\text{tw}\right] \xrightarrow{\alpha = \pi/2} \pm i (m_1+m_2) S^\text{tw}_{12}\\
...
\noindent
Suppose to have four flavours of quarks $\{\psi^1,\psi^2,\psi^3,\psi^4\}$ and a product of two Dirac bilinears of the following form:
\begin{equation*}
...
I want to express the same quantity $\mathcal{E}$ by contracting $\bar\psi^1$ with $\psi^4$ and $\bar\psi^3$ with $\psi^2$, i.e. find $M$ and $N$ such that:
\begin{equation*}
    \mathcal{E} = \left(\bar\psi^1 M \psi^4 \right)\cdot\left(\bar\psi^3 N \psi^2 \right)
\end{equation*} 
In order to build up $M$ and $N$, I need the following theorem:
...
Moreover, this is a normal basis, i.e. there exists a scalar product such that the matrices of the basis are normal:
\begin{equation*}
    \begin{aligned}
...
Then any matrix $X \in$ Mat($4 \x 4, \mathbb{C}$) can be expressed in the following way:
...
By Making the indices explicit:
\begin{equation*}
...
I rewrite it with explicit indices and I stress that $\left[\psi^2 \bar\psi^3\right]$ is a matrix:
\begin{equation*}
    \mathcal{E} = \bar\psi^1_i \Gamma^\alpha_{ij} \left[\psi^2 \bar\psi^3\right]_{jk} \Gamma^\beta_{kl} \psi^4_l 
\end{equation*}
now I express the matrix $X = \left[\psi^2 \bar\psi^3\right]$ through formula \ref{eq:fierz_decomposition}:
\begin{equation*}
    \begin{aligned}
...
Then the expression $\mathcal{E}$ becomes:
...
\end{equation}
The last formula is known under the name of \textit{Fierz transformation} of Dirac bilinears and it is usually used to re-express Fermi interactions \cite{Itzykson-Zuber}.
...
\noindent
Then I just need to apply the transformations \ref{eq:fierz_transformation} to the operators $\Theta_3$ and $\Theta_5$ defined in \ref{eq:Thetai-operators}.
In this way the colour indices $a,b$ are contracted in the same couple of fermions as the Dirac indices and the evaluation of Wick contractions is simplified.
I will develop the evaluation in the case of $\Theta_3$ and the case of $\Theta_5$ is similar.
\newline
...
\noindent
In this phantom chapter I want just to give some proofs that are not proved in the chapter of the thesis.
