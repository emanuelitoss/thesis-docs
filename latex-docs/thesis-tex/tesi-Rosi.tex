...
\begin{equation*}
    \hat B_K = \left(\frac{\overline{g}(\mu)^2}{4 \pi}\right)^{-\gamma_0/(2\beta_0)} \text{exp}\left\{ \int_0^{\overline{g}(\mu)} dg \hspace*{.5mm} \left( \frac{\gamma (g)}{\beta (g)} + \frac{\gamma_0}{\beta_0 g}\right) \right\}
\end{equation*}
\newline
...
Then the noise spinors method is extended to the case of three points correaltors: this is the most important part of the code that I wrote.
...
In the third section I will give an outline of the data analysis strategy that must be implemented to extract the continuum limit bare amplitudes $\la \bar K^0 | Q_i^{[+]} | K^0 \ra $ of the standard QCD.
...
\noindent
This section gives an overview about the analysis strategy used to recover the lattice amplitudes $\mathcal{A}_i (a)$ through the method of asymptotic behaviours outlined in section \ref{sec:asympt-behav}.
...
I start with the maximal region $[a, T-a]$ and decrease it step by step $[2a,T-2a], [3a,T-3a], \dots $ until the the fit is satisfactory - i.e. a good value of $\chi^2$ \footnote{\colr{Devo ancora capire se possa andar bene questo metodo o se è troppo approssimativo}}.
...
The coefficients $\alpha (t')$ and $\beta (t')$ must be choosen in order to make $G_\omega(t,t')$ continous and its first derivative must be an Heaviside $\theta(t-t')$.
