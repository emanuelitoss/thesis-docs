...
\begin{equation*}
    \begin{split}
        ...
        & \approx \frac{\sum_{\vec y} \la \bar K^{'0} | O_{1[+]} | K^0 \ra }{\left[ \sum_{\vec{y}} \la \Omega | A_{0,43} | \bar K^{'0} \ra \right] \left[ \sum_{\vec y} \la \Omega | A_{0,21} | \bar K^{0} \ra \right]} \approx \xi_1 B_K
    \end{split}
\end{equation*}
...
\begin{equation}\label{eq:bag-parameters-extraction}
    \begin{split}
        ...
        & \text{Continuum limit:} \quad \mathcal{B}_i (a;y_4) \xrightarrow{a \rightarrow 0} \mathcal{B}_i\bare + O(a^2)
    \end{split}
\end{equation}
...
To complete the study of the estimator, a set of $\varepsilon$s for a large amount of configurations should be derived.
\begin{table}[h!]
    \centering
...
The correlators were calculated through $N_\text{nosie} = 20$ noise spinors and the Figure \ref{fig:pureYM} show the results.
\begin{figure}[!h]
    \centering
...
On the other hand, the used configurations lives in a small lattice then the number of temporal points could be too small to fall in asymptotic regime.
...
Usually the parameters are given along with the configuration, when available and already been studied.
...
The immaginary part of these correlators have been plotted and prove that these correlators are real.
...
\begin{itemize}
    ...
    \item [-] strange quark twisted mass is chosen to be $a\mu_s = 0.1482$.
