...
\begin{abstract}
    ...
    The objective is to compute the bare Kaon oscillation amplitudes caused by each $\Theta_i$ operator:
    ...
    \newline
    Lattice simulations suffer from a discrete amount of computational issues: finite spacetime volume effects, contributions from the boundaries, lattice spacing effects, efficiency of the propagators inversion, \dots
    The lattice setup chosen for this work aims to minimize some of these effects. 
    Firstly, there is a computational problem in the generation of gauge configurations; ideally, they should be totally uncorrelated each other, but their autocorrelation times grows as the lattice spacing is reduced.
    It is proved that this problem is partially solved by choosing open boundary conditions in temporal direction.
    Secondly, these boundary conditions protects the fermion determinant from the occurrence of zero eigenvalues of the Wilson-Dirac operator.
    Open boundary conditions give an efficient alternative to twisted mass QCD - used in \cite{KMBSM} - and allow the simulation program to simulate a third flavour of sea quark: $\{u,d,s\}$.
    ...
    All the properties are described in detail and most of them are proved. At the end of the chapter I describe the asymptotic behaviour extrapolation method, widely used in quantum mechanics and QFT.
...
In both sections a short proof of the equivalence with the Wick contractions is given.
...
In fact it can be adapted to evaluate {\it every} three point correlator with a given quark content.
...
\noindent
The program is developed to generate three points correlation functions and it is based on a previous version that calculates the meson propagators.
...
The program takes as input an ensemble of sea configurations and produces the correlators of the form \ref{eq:lattice-correlators} with $x_4$ and $z_4$ fixed and $y_4$ running in the region $[z_4,x_4]$.
Instead of read some external configurations, it is also possible to set the Gauge fields to identity $U_\mu(x) = \mathbb{I}_3$ (absence of interactions) or to randomly generate them.
These two options were very useful to test the right behaviour of the simulation.
The program requires some input parameters:
\begin{itemize}
    ...
    \item [-] Informations about the noise spinor method: the number of noise vectors $N_\text{noise}$ to generate in a set and the generating distibution chosen from one of the following $U(1)$, \texttt{Gauss}, $\mathbb{Z}_2$.
    \item [-] Quark flavours informations: the critical mass $M^i_\text{cr}$ and the bare (twisted) mass parameter $\mu^i$ for each quark.
    \item [-] Dirac matrices to insert: $\Gamma_\omega$, $\Gamma_\xi$ for the sources and $\Gamma_\sigma$, $\Gamma_\rho$ for the intermediate operator. The program allows to insert different operators within the same sources.
...
The data are summed over all the spatial components.
...
\noindent
At this point I am ready to analyze three points correlators $C_i^\text{FR}(x_4,y_4,z_4)$.
...
\noindent
The program behaviour was tested both in the vacuum $U_\mu (x) = \mathbb{I}_3$ and using unphysical random configurations.
Two gauge invariance tests were made.
\newline
The first is a test on the propagator calculation.
It is set a random stochastic spinor as a pointlike spinor $\eta(x)_\alpha^a = \delta^{a,\bar a}\delta_{\alpha \bar \alpha} \delta_{x,\bar x}$ with $\bar a, \bar \alpha, \bar x$ fixed lattice, Dirac and colour indices.
By inverting the propagator on it, I got the following:
\begin{equation*}
    S(y, \bar x)[U]^{b,\bar a}_{\beta, \bar \alpha} \hspace*{1cm} \forall \hspace*{0.3mm} b,\beta,y
\end{equation*}
I evaluated the Wick contractions of the previous chapter simply by contracting the propagators with the matrices by hand, index by index.
This is done for random sea fields and their gauge transformed fields.
The check of Gauge invariance was succesfully achieved by comparing these data, as shown in Figure \ref{fig:check1}:
\begin{figure}[!h]
    \centering
    ...
    \caption{Check of Gauge invariance on the 3 points correlators. Since they are real, only the real part is plotted.
        I stress that in these data there is no physical meaning.
        The gauge fields are randomly generated, the sources are a $\gamma_5$ and an identity. The intermediate operator is a $\Psi_3$.}
    \label{fig:check1}
\end{figure}
\newline
The other test consists in the generation of a random set of gauge fields, a calculation of the correlators through the noise spinors method, a gauge transformation on the Gauge fields and a check that the two set of data coincides.
...
The gauge configurations were generated with and inverse lattice coupling $\beta = 6.0$ and in absence of quarks on the sea sector (quenched approximation).
...
On the other side, all the used parameters have no physical meaning because their determination requires a lot of work, not necessary for a simple test such that.
For this reason usually the parameters are given along with the configuration, when available.
\newline
Further developements involve the calculation of these three points correlator by using faithful sea configurations and to set the right parameters on the input files.
