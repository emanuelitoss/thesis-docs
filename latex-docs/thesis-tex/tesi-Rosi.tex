...
\begin{abstract}
    ...
    Open boundary conditions give an efficient alternative to twisted mass QCD (tmQCD) \cite{tmLQCD} - used in \cite{KMBSM} - and allow the simulation program to simulate three flavours of sea quarks, not allowed in tmQCD.
...
\end{equation*}
It is proven that the second method ensures a natural \oait, without the requirement of additive counterterms a la Symazink.
...
\begin{equation}\label{eq:matrix-elements-extraction}
    ...
    \end{split}
\end{equation}
There exists another way that does not require a fit of the two points functions to get the masses $m_{K'}$ and $m_K$.
...
\newline
Another test has been made on an ensemble of 54 gauge configurations generated using the \texttt{OpenQCD-1.2} program.
